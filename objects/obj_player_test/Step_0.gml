...
sprite_index = sprite[face];
// Shooting logic (with improved error handling)
if (cooldown_3 <= 0 && mouse_check_button(mb_right)) {
    cooldown_3 = cooldown_value_3;
    if (instance_exists(obj_gun_ogon)) { // Check if the gun object exists
        with (instance_create_layer(x, y, "Bullets_3", obj_projectile_ogon)) {
			audio_play_sound(Pew, 1, false);
            direction = obj_gun_ogon.image_angle;
            speed = 7;
        }
    } else {
        show_debug_message("obj_gun not found!");
...
// Shooting logic (with improved error handling)
if (cooldown_2 <= 0 && mouse_check_button(mb_middle)) {
